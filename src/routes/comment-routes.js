...
import { CommentPosts, deteleComments, getComments, UpdateComments, } from "../controllers/authentication/Comments/comments-controllers.js";
...
CommentRoutes.get("/getAllComments/:postId", tokenMiddleware, async (context) => {
...
CommentRoutes.delete("/deleteComments/:commentsId", tokenMiddleware, async (c) => {
    const userId = await c.get("userId");
...
CommentRoutes.patch("/updateComments/:commentsId", tokenMiddleware, async (c) => {
    const userId = await c.get("userId");
    const commentsId = await c.req.param("commentsId");
    const { content } = await c.req.json();
    try {
